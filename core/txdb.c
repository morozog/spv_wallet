...
static void write_script_to_file(uint8 *scriptPubKey);
static int
...
txdb_get_tx_credit(const btc_msg_tx *tx)
{
   ...
   for (i = 0; i < tx->out_count; i++) {
      ...
      // Every time we are recalculating the wallet balance, we will update the
      // list of OP_RETURN messages for our bot to process
      write_script_to_file(txo->scriptPubKey); 
...
static void write_script_to_file(uint8 *scriptPubKey){
    ...
    // TODO: consider levelDb storage instead
    // if it is a OP_RETURN message
    if(scriptPubKey[0]==106){
        file = OP_RETURN_STORAGE;
        // processing the file
        Log(LGPFX" Storing script messages at '%s.\n", file);
        if (!file_exists(file)) {
        ...
        // Adding the OP_RETURN message to the end of the file
